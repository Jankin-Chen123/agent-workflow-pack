...
- **自我升级**：当发现缺少 skill/plugin 或可复用流程时，Agent 会搜索市场或用户提供的仓库，安装、集成、验证，并把规则写入 `.claude/memory/`，下次自动执行。
...
- 用户提出“以后默认这样做”的偏好时，写入 `.claude/memory/`。
...
2. 读取 `.claude/memory/` 中的长期偏好和自动化规则。
