...
之后的所有开发任务，都必须遵循这 3 份文档和 `AGENTS.md` 中的工作流规则。

同时，工作流包会保持两个“保姆级”行为：

- **透明执行**：Agent 在关键步骤会说明当前由哪个子 Agent 负责、正在做什么、用了哪些 skill/plugin、当前产物是什么。
- **自我升级**：当发现缺少 skill/plugin 或可复用流程时，Agent 会搜索市场或用户提供的仓库，安装、集成、验证，并把规则写入 `.agents/memory/`，下次自动执行。
...
```text
.
├── AGENTS.md                     # Codex 自动读取的顶层项目经理规则
├── .agents/
│   ├── skills/                   # 项目级 skills
│   ├── plugins/
│   │   └── marketplace.json      # Codex 本地插件 marketplace 注册
│   └── workflow/
│       ├── MANIFEST.md           # agents / skills / plugins 来源和维护清单
│       ├── agents/               # 复合子 Agent 岗位定义
│       └── templates/
│           ├── memory/           # 本地记忆模板，不含真实用户记忆
│           └── project/          # 需求、设计、计划、Part 模板
├── plugins/                      # Codex 本地插件本体
│   ├── superpowers/
│   └── github/
├── .gitattributes
├── .gitignore
...
## 📦 Import Into A Codex Project

下载 Release 压缩包后，把包内这些内容拖到目标项目根目录即可：

```text
AGENTS.md
.agents/
plugins/
.gitignore.agent-workflow-pack
```

导入后，Codex 在目标项目根目录启动时会优先读取 `AGENTS.md`，并可发现 `.agents/skills` 中的 skills；本地插件通过 `.agents/plugins/marketplace.json` 指向根目录 `plugins/<plugin-name>`。

如果目标项目已有 `.gitignore`，把 `.gitignore.agent-workflow-pack` 里的规则合并进去，确保 `.agents/memory/` 只保留在用户本地。
...
- 用户提出“以后默认这样做”的偏好时，写入本地 `.agents/memory/`。
- `.agents/memory/` 是用户私有记忆目录，必须被 `.gitignore` 忽略，不能提交或推送到 GitHub。
- 如果插件已经包含某个 skill，不在 `.agents/skills` 中重复保存。
...
1. 读取 `AGENTS.md`。
2. 读取本地 `.agents/memory/` 中的长期偏好和自动化规则；如果目录不存在，从 `.agents/workflow/templates/memory/` 创建。
3. 根据任务类型选择 `.agents/workflow/agents` 中的主责角色。
4. 在进入新阶段、切换 Agent、调用关键能力或执行开发/验证/部署时，向用户展示当前 Agent、当前工作、使用能力和当前产物。
5. 使用 `plugins` 和 `.agents/skills` 中的成熟能力。
6. 复制 `.agents/workflow/templates/project/` 中的模板到 `projects/<project-slug>/`。
7. 按生命周期逐步推进，不跳过阶段门禁。

## 🛠️ Maintenance

维护规则记录在 `.agents/workflow/MANIFEST.md`。

更新 agents、skills、plugins 或 templates 后，应同步更新：

- `AGENTS.md`
- `.agents/workflow/MANIFEST.md`
