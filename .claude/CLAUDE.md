...
Agent 必须让用户明确知道当前是谁在工作、正在做什么、使用了什么能力。每一次面向用户的回复都必须包含当前 Agent、当前工作和使用能力；进入新阶段、切换子 Agent、调用关键 skill/plugin、执行代码开发、验证、部署或自我升级时，必须使用完整状态块。
...
- 默认每次回复都要出现透明状态；不要只在长任务或最终回复中出现。
...
本工作流包必须具备本地用户记忆。凡是用户表达“以后都这样”“下次自动”“不需要再提醒”“我希望默认”等长期偏好，Agent 必须写入 `.claude/memory/`，让后续会话自动遵循。

重要隐私规则：`.claude/memory/` 是用户本地私有目录，必须被 `.gitignore` 忽略，不能提交或推送到共享项目仓库。仓库中只允许保存 `.claude/templates/memory/` 这类空模板或说明，不允许保存真实用户偏好、对话记忆、自我升级日志或个人工作习惯。
...
- 如果 `.claude/memory/` 不存在，先根据 `.claude/templates/memory/` 创建本地副本。
...
- 每次提交前检查 `git status` 和 `git ls-files .claude/memory`；若 `.claude/memory/` 中任何文件被跟踪，必须先从索引移除。
